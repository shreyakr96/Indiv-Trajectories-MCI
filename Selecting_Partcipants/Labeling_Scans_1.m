...
while(i<length(MCI_NC))
    ...
    prev_diagnosis = MCI_NC(i,3);
    MCI_switch = 0; %should be flipped to one once the first MCI visit is encountered
    MCI_vis1 = 0;
    MCI_visfin = 0;
   
    
    if cell2mat(MCI_NC(i,1))== 4473
        disp("hi")
    end
    
    while(cell2mat(MCI_NC(i,1))== sub_RID) %Traversing through visits of the same subject
         
    nan_check2 = isnan(cell2mat(MCI_NC{i,3}));
        if (strcmp(MCI_NC{i,3},'CN') && nan_check2(1,1)~=1) %subject has a diagnosis of CN or blank diagnosis
            i = i+1;
            if (i>length(MCI_NC))
                break;
            end
        else
            if (MCI_switch == 0 && strcmp(MCI_NC{i,3},'MCI') && nan_check2(1,1)~=1)
                MCI_switch =1;
            ...
            elseif (MCI_switch==1 && nan_check2(1,1)==1 || strcmp(MCI_NC{i,3},"MCI") && MCI_switch==1)
                MCI_visfin = cell2mat(MCI_NC(i,4));
                i = i +1;
                if(i>length(MCI_NC))
                    break
...
%Computing the duration each subject spends in CN stage
i = 1;
k = 1;
CN_diff = {};
while(i<length(CN))
    sub_RID = cell2mat(CN(i,1));
    CN_vis1 = 0;
    CN_visfin = 0;
    CN_first =0; %flip to 1 for the first visit of each subject
    
    
    while(cell2mat(CN(i,1))== sub_RID) %Traversing through visits of the same subject
        if (CN_first ==0) %subject has a diagnosis of CN or blank diagnosis
          
            CN_first = 1;
            CN_vis1 = cell2mat(CN(i,4));
             i = i+1;
    ...
    CN_diff{k,1} = sub_RID;
    CN_diff{k,2} = CN_visfin - CN_vis1;
    k=k+1;
    
end

%Retaining subject Ids who have been in CN stage>=24 months

CN_fin = {};
l = 1;
for i = 1:length(CN)
    for k = 1:length(CN_diff)
    if (CN{i,1} == CN_diff{k,1} && CN_diff{k,2}>=24)
        CN_fin(l,:) = CN(i,:);
        l = l+1;
    end
    end
...
AD_IDs_new = unique(AD(:,2)); %279
MCI_NC_IDs_new = unique(MCI_NC_fin(:,2));%171
CN_IDs_new = unique(CN(:,2)); %629 
MCI_C_IDs_new = unique(MCI_C(:,2)); %158
