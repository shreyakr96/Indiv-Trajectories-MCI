...
ggplot(MCI, aes(x=Age_sc, y=F1_fit, group = RID, color = Cohort))+geom_line(size=1) + 
  ...
  geom_point(aes(x=Age_sc, y=F1_sc, group = RID, color= Cohort), size = 1.5, stroke = 0, colour = ifelse(MCI$Cohort == "MCI_C", 'darkblue','firebrick4'),size = 1) + 
  geom_abline(slope = F1_MCI_C_slope, intercept = F1_MCI_C_intercept, colour = 'darkblue',size = 1.5) +
  geom_abline(slope = F1_MCI_NC_slope, intercept = F1_MCI_NC_intercept, colour = 'firebrick4',size = 1.5) +
...
ggplot(MCI, aes(x=Age_sc, y=F2_fit, group = RID, color = Cohort))+geom_line(size=1) + 
  geom_point(colour = ifelse(MCI$Cohort == "MCI_C", 'darkslategrey','red'),size = 1) +  
  geom_point(aes(x=Age_sc, y=F2_sc, group = RID, color= Cohort), size = 1.5, stroke = 0, colour = ifelse(MCI$Cohort == "MCI_C", 'darkblue','firebrick4'),size = 1) + 
  geom_abline(slope = F2_MCI_C_slope, intercept = F2_MCI_C_intercept, colour = 'darkblue',size = 1.5) +
  geom_abline(slope = F2_MCI_NC_slope, intercept = F2_MCI_NC_intercept, colour = 'firebrick4',size = 1.5) +
...
p1<- ggplot(MCI, aes(x=Age_sc, y=F3_fit, group = RID, color = Cohort))+geom_line(size=1) + 
  geom_point(colour = ifelse(MCI$Cohort == "MCI_C", 'darkslategrey','red'),size = 1) +  
  geom_point(aes(x=Age_sc, y=F3_sc, group = RID, color= Cohort), size = 1.5, stroke = 0, colour = ifelse(MCI$Cohort == "MCI_C", 'darkblue','firebrick4'),size = 1) + 
  geom_abline(slope = F3_MCI_C_slope, intercept = F3_MCI_C_intercept, colour = 'darkblue',size = 1.5) +
  geom_abline(slope = F3_MCI_NC_slope, intercept = F3_MCI_NC_intercept, colour = 'firebrick4',size = 1.5) +
...
F1_rand_eff_updated <- left_join(F1_rand_eff, new_dataframe %>% select(RID, Age), by = "RID")
model_F1 <- lm(Age ~ Age_sc, data = F1_rand_eff_updated)
# Summary of the regression model
summary(model_F1)

# Assuming new_dataframe_2 and Hip_rand_eff are your dataframes
F2_rand_eff_updated <- left_join(F2_rand_eff, new_dataframe %>% select(RID, Age), by = "RID")
model_F2 <- lm(Age ~ Age_sc, data = F2_rand_eff_updated)
# Summary of the regression model
summary(model_F2)

# Assuming new_dataframe_2 and Hip_rand_eff are your dataframes
F3_rand_eff_updated <- left_join(F3_rand_eff, new_dataframe %>% select(RID, Age), by = "RID")
model_F3<- lm(Age ~ Age_sc, data = F3_rand_eff_updated)
# Summary of the regression model
summary(model_F3)
